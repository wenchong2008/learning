...
from lib.commons import log_write
...
class FTPServer(object):
    ...
    def _get_dir(self,option):
        ...
        if len(abspath) < len(self.homedir):
    ...
    def put(self, option):
        ...
        try:
            if option['action'] == 'put' and 'filename' in option and 'size' in option:
                self.send_data(json.dumps({'status':'start'}))
                receive_size = 0
                tmp_filename = option['filename']+".tmp"
                # 组合上传文件存放的真实路径 = 用户家目录 + 当前目录 + 临时文件名
                tmp_filename = os.path.join(self.homedir, re.sub('^/','',self.current_dir), tmp_filename)
                # 接收文件
                ...
                if receive_size == option['size']:
                    os.rename(tmp_filename, re.sub('\.tmp$','', tmp_filename))
    ...
    def get(self, option):
        ...
        try:
            if option['action'] == 'get' and 'filename' in option:
                ...
                # 3、判断文件
                if len(os.path.dirname(download_file)) < len(self.homedir) or not os.path.exists(download_file) or not os.path.isfile(download_file):
                    ...
                    # 4、发送文件名称和大小给客户端
                    self.send_data(json.dumps({'filename': option['filename'],'size':os.path.getsize(download_file)}))
                    # 5、接收客户端发送的状态
